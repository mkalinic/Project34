...
controllers.Project_controller = function ($scope, $rootScope, $location, $translate, $filter, $window, ErrorHandler, Project, User, UsersProject, Milestone, Phase, TextBlock, TextBlockFile, $routeParams, Upload, Post, Checklist, $timeout, $http, Ajax) {
    ...
    $scope.init = function () {
        ...
        User.GetAll(function (users) {
            $scope.AllUsers = users;console.log('-----  $scope.AllUsers ', $scope.AllUsers);
        }, function () {});
    };

    var arangeUsers = function arangeUsers() {
        if ($scope.Project.UsersProjects == null) $scope.Project.UsersProjects = [];

        for (var i = 0; i < $scope.Project.UsersProjects.length; i++) {
            if ($scope.Project.UsersProjects[i].userType == "IGG") $scope.IGGUsersProjects.push($scope.Project.UsersProjects[i]);else $scope.UsersProjects.push($scope.Project.UsersProjects[i]);
        }
    ...
    $scope.getUserForThisProjectById = function (id) {
        ...
        if ($scope.Project.UsersProjects) for (var i = 0; i < $scope.Project.UsersProjects.length; i++) {
    ...
    $scope.SendNoticesToPeopleOnProject = function () {
        ...
        Project.SendNoticesToPeopleOnProject($scope.Project.ID, function (retval) {
    ...
    var changeNotificationText = function changeNotificationText(newText) {
    ...
    $scope.SendPostResponse = function (NewPostText, NewPostImage, NewDefinitionText) {
        ...
        if (!$scope.Project.UsersProjects || $scope.Project.UsersProjects.length < 1) {
            if (!confirm($filter('translate')('there are no users on this project, are you sure you want to save this post?'))) return;
        }
        ...
        Project.SendPostResponse($scope.Project.ID, theNewPost, $scope.NewPost, function (post) {
    ...
    $scope.removeTextBlock = function (theTextBlock) {
        ...
        TextBlock['delete'](theTextBlock, function (response) {
            //   removeObjectFromArray($scope.Project.TextBlocks, $scope.Project.TextBlocks[i]);
        }, function (err) {});
    ...
    $scope.addTextBlockFile = function (textBlockFile, textBoxID) {
        ...
        for (var i = 0; i < $scope.Project.TextBlocks.length; i++) {
            if ($scope.Project.TextBlocks[i].ID == textBoxID) {
                ...
                //------- save the textbox:
                TextBlock.save($scope.Project.TextBlocks[i], function (response) {
    ...
    /** Object must have ID */
    var removeObjectFromArray = function removeObjectFromArray(arrayOfObjects, objectToRemove) {
    ...
    /** Object must have ID */
    var removeObjectFromArraybyID = function removeObjectFromArraybyID(arrayOfObjects, ID) {
        if (!arrayOfObjects) return;
        for (var i = 0; i < arrayOfObjects.length; i++) {
    ...
    $scope.datePickers = [];
    var AddDatePickers = function AddDatePickers() {
        if (!$scope.Project.TextBlocks) return;
        for (var i = 0; i < $scope.Project.TextBlocks.length; i++) {
            $scope.datePickers.push(new DatePicker());
        }
    ...
    $scope.addUser = function () {
        if ($scope.Project.ID > 0) {
            ...
            Project.SendNewUserInProjectMail($scope.Project.ID, $scope.NewUserProject, function (retval) {
    ...
    $scope.deleteEditedUserProject = function () {
        ...
        if ($scope.Project.ID > 0) {
            // if editing existing project
            UsersProject['delete']($scope.EditedUserProject, function (response) {
    ...
    $scope.saveEditedUserProject = function () {
        ...
        if ($scope.Project.ID > 0) {
            // if editing existing project
            UsersProject.save($scope.EditedUserProject, function (response) {
    ...
    var hideAllModalBackgrounds = function hideAllModalBackgrounds() {
    ...
    $scope.removeIGGer = function (igger) {
        if ($scope.Project.ID > 0) {
            // if editing existing project
            UsersProject['delete'](igger, function (response) {
    ...
    $scope.addIGGer = function (iggerID) {
        ...
        if ($scope.Project.ID > 0) {
            // if editing existing project
            UsersProject.AddIggerToProject(iggerID, $scope.Project.ID, function (response) {
    ...
    $scope.OpenVault = function () {
        Project.OpenVault($scope.Project.ID, function (response) {
    ...
    $scope.upItem = function (ID, itemOrder) {
        ...
        for (var i = 0; i < $scope.Checklist.length; i++) {
            if ($scope.Checklist[i].ID == ID) {
                ...
                $scope.Checklist[i].itemOrder++;
                Checklist.save($scope.Checklist[i], function () {
    ...
    $scope.removeChecklistItem = function (ID) {
        ...
        Checklist.removeItem(ID, function (response) {
    ...
    var GetChecklistForProject = function GetChecklistForProject(projectID) {
        // console.log('GetChecklistForProject ', projectID);
        Checklist.GetForProject(projectID, function (response) {
    ...
    $scope.addChecklistItem = function () {
        ...
        Checklist.save(newOne, function () {
            GetChecklistForProject($scope.Project.ID);$scope.NewChecklistItem = "";
        }, function () {});
    ...
    $scope.showChangeName = function (elem, ID) {
        ...
        for (var i = 0; i < 1000; i++) if ($scope.showItemChange[i]) $scope.showItemChange[i] = false;
